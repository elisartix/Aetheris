...
from aetheris_tl.errors import WebpageMediaEmptyError
from aetheris_tl.types import InputMediaWebPage
...
@loader.tds
class AetherisInfoMod(loader.Module):
    """Show userbot info"""

    strings = {"name": "AetherisInfo"}
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            ...
            loader.ConfigValue(
                "banner_url",
                "https://elisart.ru/assets/aetheris_banner.png",
    ...
    async def _render_info(
        ...
        me = (
            '<b><a href="tg://user?id={}">{}</a></b>'.format(
                self._client.aetheris_me.id,
                utils.escape_html(get_display_name(self._client.aetheris_me)),
            )
            .replace("{", "")
            .replace("}", "")
        )
    ...
    async def infocmd(self, message: Message):
        ...
        if banner and self.config["quote_media"] is True:
            media = InputMediaWebPage(str(banner), optional=True)

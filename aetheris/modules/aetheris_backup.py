...
@loader.tds
class AetherisBackupMod(loader.Module):
    """Handles database and modules backups"""

    strings = {"name": "AetherisBackup"}
    ...
    async def handler(self):
        try:
                ...
                reply_markup=self.inline.generate_markup(
                    [
                        [
                            {
                                "text": "↪️ Restore this",
                                "data": "aetheris/backupall/restore/confirm",
                            }
                        ]
                    ]
                ),
            ...
            logger.exception("AetherisBackup failed")
            await asyncio.sleep(60)

    @loader.callback_handler()
    async def restore(self, call: BotInlineCall):
        if not call.data.startswith("aetheris/backupall/restore"):
            return

        if call.data == "aetheris/backupall/restore/confirm":
            await utils.answer(
                call,
                "❓ <b>Are you sure?</b>",
                reply_markup={
                    "text": "✅ Yes",
                    "data": "aetheris/backupall/restore",
                },
            )
            return
        ...
        try:
            ...
            with zipfile.ZipFile(zipfile_bytes) as zf:
                ...
                with contextlib.suppress(KeyError):
                    db_data["aetheris.inline"].pop("bot_token")
    ...
    def _convert(self, backup):
        fixed = re.sub(r"(hikka\.)(\S+\":)", lambda m: "aetheris." + m.group(2), backup)
    ...
    async def restoredb(self, message: Message):
        ...
        with contextlib.suppress(KeyError):
            decoded_text["aetheris.inline"].pop("bot_token")
    ...
    async def backupall(self, message: Message):
        ...
        archive.name = f"aetheris-{datetime.datetime.now():%d-%m-%Y-%H-%M}.backup"

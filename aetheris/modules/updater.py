...
# 🌐 https://github.com/elisartix/Aetheris
# You can redistribute it and/or modify it under the terms of the GNU AGPLv3
# 🔑 https://www.gnu.org/licenses/agpl-3.0.html
...
from aetheris_tl.tl.functions.messages import (
...
NO_GIT = os.environ.get("AETHERIS_NO_GIT") == "1"
...
class UpdaterMod(loader.Module):
    """Updates itself, tracks latest Aetheris releases, and notifies you, if update is required"""
    ...
    async def poller_announcement(self):
        async with aiohttp.ClientSession() as session:
            try:
                url = "https://api.github.com/repos/elisartix/assets/contents/aetheris/announcment.txt"
                r = await session.get(
                    url,
        ...
        if self._pending not in {current, self._notified}:
            if not self.config["autoupdate"]:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        r = await session.get(
                            url=f"https://api.github.com/repos/elisartix/Aetheris/contents/aetheris/version.py?ref={version.branch}",
                            headers={"Accept": "application/vnd.github.v3.raw"},
                        )
                        text = await r.text()
            ...
            if manual_update:
                m = await self.inline.bot.send_photo(
                    self.tg_id,
                    "https://raw.githubusercontent.com/elisartix/assets/refs/heads/main/aetheris/updated.png",
    ...
    async def update_call(self, call: InlineCall):
        ...
        if NO_GIT:
            ...
            return
        if call.data not in {"aetheris/update", "aetheris/ignore_upd"}:
            return

        if call.data == "aetheris/ignore_upd":
    ...
    async def restart_common(
        ...
        msg_obj = await utils.answer(
            ...
            self.strings["restarting_caption"].format(
                ...
                if self._client.aetheris_me.premium
                else "Aetheris"
            ),
        )
            ...
            try:
                with Repo(os.path.dirname(utils.get_base_dir())) as repo:
                    ...
                    if origin.url != self.config["GIT_ORIGIN_URL"]:
                        logger.warning(
                            "Updater origin %s differs from configured source %s",
                            origin.url,
                            self.config["GIT_ORIGIN_URL"],
                        )
    ...
    async def client_ready(self):
            ...
            [
                {
                    ...
                    "data": "aetheris/update",
                    "style": "primary",
                },
                {
                    "text": self.strings["ignore"],
                    "data": "aetheris/ignore_upd",
                    "style": "danger",
                },
            ]
        )
        ...
        if not self.config["autoupdate"] and not self.get("autoupdate_answered", False):
            await self.inline.bot.send_photo(
                self.tg_id,
                photo="/root/Aetheris/aetheris_info.png",
        ...
        if filters:
            ...
            for folder in filters:
                ...
                if title:
                    ...
                    if str(raw_title).strip() == "Aetheris":
                        aetheris_f = True

        if aetheris_f is True:
            return
        else:
            try:
                await self._client(
                    UpdateDialogFilterRequest(
                        ...
                        DialogFilter(
                            folder_id,
                            title=TextWithEntities(text="Aetheris", entities=[]),
                            pinned_peers=(
                                [
                            ...
                            include_peers=[
                                ...
                                if "aetheris" in dialog.name
                                or "Aetheris" in dialog.name
                ...
                logger.critical(
                    "Can't create Aetheris folder. Possible reasons are:\n"
    ...
    async def ubstop(self, message: Message):
        ...
        await self.inline.form(
            ...
            text=self.strings["stop_ub_confirm"].format(
                utils.get_platform_emoji()
                if self.client.aetheris_me.premium
                else "Aetheris"
            ),
            reply_markup=[
                [

...
from ..types import AetherisReplyMarkup, ListLike
...
def smart_split(
    ...
    :example:
        >>> utils.smart_split(
            *aetheris_tl.extensions.html.parse(
                "<b>Hello, world!</b>"
            )
        )
        <<< ["<b>Hello, world!</b>"]
...
async def answer(
    ...
    response: str,
    *,
    reply_markup: AetherisReplyMarkup | None = None,
    **kwargs,
) -> InlineCall | InlineMessage | Message:
    """
    Use this to give the response to a command
    :param message: Message to answer to. Can be a tl message or aetheris inline object
    ...
    if reply_markup is not None:
        ...
        if reply_markup:
            ...
            try:
                ...
                logger.warning(
                    "Inline response failed; falling back to a plain Telegram message",
                    exc_info=True,
                )
            reply_markup = None
    ...
    parse_mode = aetheris_tl.utils.sanitize_parse_mode(
        kwargs.pop(
            "parse_mode",
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "aetheris_grepped"):
            try:

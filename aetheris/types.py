...
from aetheris_tl.tl.functions.account import UpdateNotifySettingsRequest
...
class Module:
    ...
    @property
    def aetheris_commands(self) -> dict[str, Command]:
        """List of commands that module supports"""
        return get_commands(self)
    ...
    @property
    def aetheris_inline_handlers(self) -> dict[str, Command]:
        """List of inline handlers that module supports"""
        return get_inline_handlers(self)
    ...
    @property
    def aetheris_callback_handlers(self) -> dict[str, Command]:
        """List of callback handlers that module supports"""
        return get_callback_handlers(self)
    ...
    @property
    def aetheris_watchers(self) -> dict[str, Command]:
        """List of watchers that module supports"""
        return get_watchers(self)
    ...
    @aetheris_commands.setter
    def aetheris_commands(self, _):
        pass
    ...
    @aetheris_inline_handlers.setter
    def aetheris_inline_handlers(self, _):
        pass
    ...
    @aetheris_callback_handlers.setter
    def aetheris_callback_handlers(self, _):
        pass
    ...
    async def _decline(
        ...
        self._db.set(
            "aetheris.main",
            "declined_joins",
            list(set(self._db.get("aetheris.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def request_join(
        ...
        match channel:
            ...
            case _ if channel.id in self._db.get("aetheris.main", "declined_joins", []):
                if assure_joined:
                    raise LoadError(
        ...
        self.aetheris_wait_channel_approve = (
            self.__class__.__name__,
            channel,
            reason,
        )
        ...
        with contextlib.suppress(AttributeError):
            delattr(self, "aetheris_wait_channel_approve")
    ...
    async def import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?aetheris_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?aetheris_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Aetheris version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"aetheris.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("aetheris.main", "stats", True)
            and url is not None

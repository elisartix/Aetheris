    ...
    try:
        match name:
            case s if s.startswith("telethon"):
                return native_import("aetheris_tl" + name[8:], *args, **kwargs)
            case s if s.startswith("hikkatl"):
                return native_import("aetheris_tl" + name[7:], *args, **kwargs)
            case s if s.startswith("hikkalls"):
                return native_import(name, *args, **kwargs)
            case s if s.startswith("hikka"):
                return native_import("aetheris" + name[5:], *args, **kwargs)
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?aetheris$", from_id=00000000)
...
class Modules:
    ...
    def __init__(
        ...
        self.watchers = []
        self._log_handlers = []
        self.module_failures: dict[int, str] = {}
        self.failed_module_count = 0
        self._core_commands = []
        self.__approve = []
        ...
        self.client.aetheris_inline = self.inline
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.aetheris_commands)
                inline_handlers.update(module.aetheris_inline_handlers)
                callback_handlers.update(module.aetheris_callback_handlers)
                watchers.extend(module.aetheris_watchers.values())
    ...
    def register_commands(self, instance: Module):
        ...
        for _command, cmd in instance.aetheris_commands.items():
    ...
    def register_inline_stuff(self, instance: Module):
        ...
        for name, func in instance.aetheris_callback_handlers.copy().items():
    ...
    def register_watchers(self, instance: Module):
        ...
        for _watcher in instance.aetheris_watchers.values():
    ...
    async def send_ready_one_wrapper(self, *args, **kwargs):
        ...
        if mod is not None:
            self.module_failures.pop(id(mod), None)
        try:
            await self.send_ready_one(*args, **kwargs)
        except Exception as e:
            if mod is not None:
                self.module_failures[id(mod)] = f"{type(e).__name__}: {e}"
    ...
    async def send_ready_one(
        ...
        try:
            ...
            logger.exception(
                (
                    "Failed to initialize module %s due to %s; isolating module"
                    " and keeping the rest of Aetheris running"
                ),
                mod,
                e,
            )
            # A broken user module must not take down core dispatch or cancel
            # sibling module initialization. Keep the object visible to .health.
            if mod.__origin__.startswith("<core"):
                self.module_failures[id(mod)] = f"{type(e).__name__}: {e}"
                self.unregister_commands(mod, "failed initialization")
                self.unregister_watchers(mod, "failed initialization")
                self.unregister_raw_handlers(mod, "failed initialization")
                self.unregister_bot_update_handlers(mod, "failed initialization")
            else:
                with contextlib.suppress(ValueError):
                    self.modules.remove(mod)
                self.module_failures[id(mod)] = f"{type(e).__name__}: {e}"
                self.failed_module_count += 1
            return
    ...
    def health_snapshot(self) -> dict[str, int]:
        return module_health_snapshot(
            self.modules, self.module_failures, self.failed_module_count
        )

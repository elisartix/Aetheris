...
from aetheris_tl.errors.rpcerrorlist import YouBlockedUserError
...
class TokenObtainment(InlineUnit):
    async def _create_bot(self: "InlineManager"):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("aetheris.inline", "custom_bot", False):
                username = self._db.get("aetheris.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
            ...
            try:
                ...
                if "DOCKER" in os.environ:
                    m = await conv.send_file(
                        "https://raw.githubusercontent.com/coddrago/Aetheris/refs/heads/master/assets/aetheris-ava.png"
                    )
                else:
                    m = await conv.send_file(
                        main.BASE_PATH / "assets" / "aetheris-ava.png"
                    )
                r = await conv.get_response()
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    ...
                    if self._db.get("aetheris.inline", "custom_bot", False) and (
                        self._db.get("aetheris.inline", "custom_bot", False) != btn_text
                    ):
                        continue

                    if not self._db.get("aetheris.inline", "custom_bot", False) and not (
                    ...
                    self._db.set("aetheris.inline", "bot_token", token)
                    self._token = token

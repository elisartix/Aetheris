...
from aetheris_tl.tl.types import UpdateBotInlineSend
...
class Events(InlineUnit):
    async def _message_handler(self: "InlineManager", message):
        ...
        match True:
            case _ if (
                ...
                or wrapped_message.text == "/start aetheris init"
            ):
                return
    ...
    async def _query_help(self: "InlineManager", inline_query: InlineQuery):
        ...
        for name, fun in self._allmodules.inline_handlers.items():
            ...
            try:
                thumb = getattr(fun, "thumb_url", None) or fun.__self__.aetheris_meta_pic
            except Exception:
                thumb = None

...
class MessageEditor:
    def __init__(
        self,
        message: aetheris_tl.tl.types.Message,
        command: str,
        config,
        strings,
        request_message,
    ...
    async def redraw(self):
        ...
        with contextlib.suppress(aetheris_tl.errors.rpcerrorlist.MessageNotModifiedError):
            try:
                self.message = await utils.answer(self.message, text)
            except aetheris_tl.errors.rpcerrorlist.MessageTooLongError as e:
                logger.error(e)
                logger.error(text)
...
class SudoMessageEditor(MessageEditor):
    ...
    async def update_stderr(self, stderr):
        ...
        if any(lastlines[0] == i for i in self.PASS_REQ) and self.state == 0:
            ...
            text = self.strings["auth_needed"].format(self.message.client.aetheris_me.id)

            try:
                await utils.answer(self.message, text)
            except aetheris_tl.errors.rpcerrorlist.MessageNotModifiedError as e:
                logger.debug(e)
            ...
            self.message.client.add_event_handler(
                ...
                aetheris_tl.events.messageedited.MessageEdited(chats=["me"]),
            )
    ...
    async def on_message_edited(self, message):
        ...
        if hash_msg(message) == hash_msg(self.authmsg):
            ...
            except aetheris_tl.errors.rpcerrorlist.MessageNotModifiedError:
...
class RawMessageEditor(SudoMessageEditor):
    ...
    async def redraw(self):
        ...
        with contextlib.suppress(
            aetheris_tl.errors.rpcerrorlist.MessageNotModifiedError,
            aetheris_tl.errors.rpcerrorlist.MessageEmptyError,
            ValueError,
        ):
            try:
                await utils.answer(self.message, text)

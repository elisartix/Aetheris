...
import aiohttp
from aetheris_tl import events
from aetheris_tl.errors import (
    ApiIdInvalidError,
...
from aetheris_tl.errors.rpcerrorlist import (
    AuthKeyUnregisteredError,
    YouBlockedUserError,
)
from aetheris_tl.network.connection import (
    ConnectionTcpFull,
    ConnectionTcpMTProxyRandomizedIntermediate,
)
from aetheris_tl.password import compute_check
from aetheris_tl.sessions import MemorySession, SQLiteSession
from aetheris_tl.tl.functions.account import GetPasswordRequest
from aetheris_tl.tl.functions.auth import CheckPasswordRequest
...
class Aetheris:
    ...
    def _migrate_sessions(self):
        ...
        with os.scandir(BASE_DIR) as entries:
            legacy = [
                ...
                if entry.is_file()
                and entry.name.startswith("aetheris-")
    ...
    async def save_client_session(
        ...
        *,
        ...
        client.aetheris_db = database.Database(client)
        await client.aetheris_db.init()

        try:
            db = client.aetheris_db
            existing = db.get("aetheris.inline", "custom_bot", False)
        except Exception:
            existing = False
        ...
        if (
            ...
            while bot := input(
                "You can enter a custom bot username or leave it empty and Aetheris will generate a random one: "
            ):
                bot = bot.strip()
                bot = bot.lstrip("@")
                if any(
                ...
                try:
                    if await self._check_bot(client, bot):
                        db.set("aetheris.inline", "custom_bot", bot)
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            pref = client.aetheris_db.get("aetheris.main", "command_prefix", None)
            ...
            try:
                ...
                await client.aetheris_inline.bot.send_photo(
                    log_chat_id,
                    "https://raw.githubusercontent.com/elisartix/assets/refs/heads/main/aetheris/aetheris_started.png",
                    caption=(
                        "{} <b>{} started!</b>\n\n<tg-emoji emoji-id=5231065262228250587>⚙</tg-emoji> <b>GitHub commit SHA: <a"
                        ' href="https://github.com/elisartix/Aetheris/commit/{}">{}</a></b>\n<tg-emoji emoji-id=5873225338984599714>🔎</tg-emoji>'
                        " <b>Update status: {}</b>\n<tg-emoji emoji-id=5870903672937911120>🕶</tg-emoji> <b>Prefix:</b> <code>{}</code>"
                    ).format(
                        (
                            utils.get_platform_emoji()
                            if client.aetheris_me.premium is True
                            else "🪐 Aetheris"
                        ),
                        ".".join(list(map(str, list(__version__)))),
                        build,
            ...
            logging.debug(
                ...
                client.tg_id,
                client.aetheris_db.get(__name__, "command_prefix", False) or ".",
            )
        except Exception:

...
from aetheris_tl import TelegramClient, events
...
from aetheris_tl.sessions import SQLiteSession
...
from aetheris_tl.utils import get_display_name
...
class InlineManager(
    ...
    :type client: aetheris.tl_cache.CustomTelegramClient
    :type db: aetheris.database.Database
    :type allmodules: aetheris.loader.Modules
    """
    ...
    def __init__(
        ...
        self._token = db.get("aetheris.inline", "bot_token", False)
    ...
    def _cleanup_stale_bot_sessions(self, bot_uid: str):
        prefix = f"aetheris-{self._me}-bot-"
    ...
    async def register_manager(
        ...
        self._bot_client = TelegramClient(
            SQLiteSession(
                os.path.join(main.SESSIONS_DIR, f"aetheris-{self._me}-bot-{bot_uid}")
            ),
            self._client.api_id,
        ...
        try:
            ...
            self._bot_client.aetheris_me = bot_me
        ...
        for folder in _folders.filters:
            if getattr(folder, "title", None) == "Aetheris":
                if any(
                    [
    ...
    async def _ping_bot(
        ...
        try:
            m = await self._client.send_message(self.bot_username, "/start aetheris init")
        except (InputUserDeactivatedError, ValueError):
            self._db.set("aetheris.inline", "bot_token", None)
            self._token = False
            ...
            try:
                m = await self._client.send_message(
                    self.bot_username, "/start aetheris init"
                )
            except Exception:

...
class Quickstart(loader.Module):
    ...
    async def client_ready(self):
        self.text = lambda: self.strings["base"].format(
            ...
            if self.client.aetheris_me.premium is True
            else "Aetheris"
        )

        try:
            content_channel = None
            existing_channel_id = self.db.get("aetheris.forums", "channel_id", None)
            ...
            if existing_channel_id:
                try:
                    ...
                    content_channel = None
                    self.db.set("aetheris.forums", "forums_cache", {"aetheris-userbot": {}})
            ...
            if not content_channel:
                async for dialog in self.client.iter_dialogs():
                    if dialog.title and "aetheris-userbot" in dialog.title.lower():
                        ...
                        self.db.set(
                            "aetheris.forums", "channel_id", int(dialog.entity.id)
                        )
                        break
            ...
            forum_entity = None
            existing_forum_id = self.db.get("aetheris.forums", "forum_id", None)
            ...
            if not forum_entity:
                try:
                    if not (
                        ...
                        from aetheris_tl.tl.functions.channels import ToggleForumRequest
                    ...
                    self.db.set("aetheris.forums", "forum_id", int(content_channel.id))
                except Exception:
                    forum_entity = content_channel
            ...
            required_topics = [
                (
                    "Assets",
                    "🌆 Your Aetheris assets will be stored here",
                    5877307202888273539,
                ),
                (
                    "Backups",
                    "💾 Your Aetheris backups will be stored here",
                    5877307202888273539,
                ),
            ]
        ...
        await self.request_join(
            "aetheris_talks",
            "Aetheris help is only available in this chat. By agreeing to join the chat, you agree to the Aetheris federation rules and if you violate them, you will be permanently banned.",
        )
    ...
    async def lang(self, call: BotInlineCall):
        if not call.data.startswith("aetheris/lang/"):
            return

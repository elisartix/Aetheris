...
from aetheris_tl.errors.common import ScamDetectionError
from aetheris_tl.errors.rpcerrorlist import MediaCaptionTooLongError
from aetheris_tl.tl.functions.channels import JoinChannelRequest
from aetheris_tl.tl.types import Channel, InputMediaWebPage
...
class LoaderMod(loader.Module):
    ...
    async def download_and_install(
        ...
        try:
            ...
            if isinstance(r, bytes) and r.startswith(b"PK\x03\x04"):
                installed = await self.install_package(
                    r,
                    message,
                    origin=url,
                )
    ...
    async def loadmod(self, message: Message):
        ...
        if doc[:4] == b"PK\x03\x04":
            await self.install_package(doc, message)
            return
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?aetheris_min", doc):
            ver = re.search(r"# ?scope: ?aetheris_min ((?:\d+\.){2}\d+)", doc).group(1)
            ver_ = tuple(map(int, ver.split(".")))
            if main.__version__ < ver_:
                logger.error(
                    "Module %s requires Aetheris %s, current version is %s",
                    module_label,
                    ver,
        ...
        module_name = f"aetheris.modules.{uid}"
        ...
        try:
            try:
                ...
                async def inner_proxy():
                    ...
                    while True:
                        if hasattr(instance, "aetheris_wait_channel_approve"):
                            if message:
                                (
                                    module,
                                    channel,
                                    reason,
                                ) = instance.aetheris_wait_channel_approve
                                message = await utils.answer(
                                    message,
        ...
        instance.aetheris_meta_pic = next(
            (
        ...
        if pack_url and (
            transations := await self.allmodules.translator.load_module_translations(
                pack_url,
                loader.MODULES_LANGPACKS_PATH / f"{instance.__class__.__name__}.yml",
            )
        ):
        ...
        try:
            developer_entity = await (
                ...
                if (
                    developer in self._client.aetheris_entity_cache
                    and getattr(
    ...
    async def install_package(
        self,
        data: bytes,
        message: Message | InlineCall | None = None,
        origin: str = "<string>",
    ) -> bool:
        """Install a multi-file module from a zip archive into its own folder.

        The archive is unpacked to ``loaded_modules/<ClassName>_<tg_id>/`` and
        loaded as a real python package, so module files can use relative
        imports (``from . import helper``) or plain imports (``import helper``).
        Single-file modules are not affected.
        """
        import tempfile
        ...
        try:
            ...
            uid = None
            if origin not in ("<string>", None):
                base = origin.split("/")[-1]
                uid = re.sub(r"\.zip$", "", base).replace("%", "%%").replace(".", "%d")

            with tempfile.TemporaryDirectory() as tmp:
                tmp_pkg = os.path.join(tmp, "pkg")
                entry = loader._extract_module_package(data, tmp_pkg)

                if not entry:
                    raise ValueError("No python module file found in archive")

                source = Path(tmp_pkg, entry).read_text(encoding="utf-8")
                ...
                if uid is None:
                    try:
                        ...
                        uid = "__extpkg_" + str(uuid.uuid4())

                if re.search(r"# ?scope: ?aetheris_min", source):
                    ver_match = re.search(
                        r"# ?scope: ?aetheris_min ((?:\d+\.){2}\d+)", source
                    )
                    if not ver_match:
                        logger.error(
                            "Package %s has malformed aetheris_min scope",
                            module_label,
                        )
                        return False
                    ...
                    if main.__version__ < ver_:
                        logger.error(
                            "Package %s requires Aetheris %s, current version is %s",
                            module_label,
                            ver,
                            ".".join(map(str, main.__version__)),
                        )
                ...
                try:
                    requirements = list(
                        filter(
                            ...
                            map(
                                str.strip,
                                loader.VALID_PIP_PACKAGES.search(source)[1].split(),
                            ),
                        )
                    )
                except TypeError:
                ...
                if requirements:
                    ...
                    if not result:
                        logger.error(
                            "Package %s requirements failed to install: %s",
                            module_label,
                            requirements,
                        )
                        if message is not None:
                            await utils.answer(
                                message, self.strings["requirements_failed"]
                            )
                        return False
                ...
                # Drop any previously loaded instance with the same class name
                with contextlib.suppress(Exception):
                    await self.allmodules.unload_module(uid)

                # Clear a stale single-file copy with the same name
                ...
                shutil.copytree(tmp_pkg, pkg_dir)

                module_name = f"aetheris.external.{uid}"

                try:
                    spec = ModuleSpec(
                        module_name,
                        loader.StringLoader(
                            source, f"<external {module_name}>"
                        ),
                        origin=f"<external {module_name}>",
                    )
                    ...
                    loader._ensure_external_namespace()
                    loader._external_packages_finder.register(uid, pkg_dir)
                    ...
                    logger.error(
                        "Package %s failed security checks: %s", module_label, e
                    )
                    with contextlib.suppress(Exception):
                        await self.allmodules.unload_module(uid)
                ...
                try:
                    ...
                    logger.error(
                        "Package %s failed during ready security checks: %s",
                        module_label,
                        e,
                    )
                    with contextlib.suppress(Exception):
                    ...
                    logger.exception(
                        "Package %s threw during ready stage", module_label
                    )
                    return False

                self.update_modules_in_db()

                logger.info("Installed module package %s into %s", uid, pkg_dir)

                if isinstance(message, Message):
                    await utils.answer(
                        message,
                        self.strings["package_loaded"].format(uid),
                    )

                return True
        except Exception:
            logger.exception("Failed to install module package %s", module_label)
    ...
    def _is_core_module(self, module) -> bool:
        ...
        if not module_name.startswith("aetheris.modules."):
            return False
    ...
    async def _inline__clearmodules(self, call: InlineCall):
        ...
        for file in os.scandir(loader.LOADED_MODULES_DIR):
            try:
                if file.is_dir() and file.name != "langpacks":

...
class CoreMod(loader.Module):
    ...
    @loader.command(
        ...
        en_doc="Information of Aetheris",
        ua_doc="Інформація про Хероку",
        de_doc="Informationen über Aetheris",
    )
    async def aetheriscmd(self, message: Message):
        ...
        platform = (
            ...
            else "🪐 <b>Aetheris userbot</b>"
        )

        if self.config["rich_mode"]:
            rich_message = self.strings["rich_aetheris_message"].format(
                platform=platform,
        ...
        await utils.answer(
            ...
            self.strings["aetheris"].format(
                ...
                *version.__version__,
                ...
                f"{aetheris_tl.__version__} #{aetheris_tl.tl.alltlobjects.LAYER}",
            )
            + (branch_text)
    ...
    async def togglecmdcmd(self, message: Message):
        ...
        if cmd.lower() not in [c.lower() for c in mod_inst.aetheris_commands.keys()]:

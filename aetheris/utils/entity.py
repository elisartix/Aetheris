...
from aetheris_tl.tl.functions.channels import CreateChannelRequest, EditPhotoRequest
...
parser = aetheris_tl.utils.sanitize_parse_mode("html")
    ...
    if _folder:
        ...
        try:
            folder = next(
                ...
                if not isinstance(folder, aetheris_tl.tl.types.DialogFilterDefault)
...
async def asset_forum_topic(
    ...
    async def create_topic() -> ForumTopic:
        result = await client(
            CreateForumTopicRequest(
                peer=entity,
                title=title,
                icon_emoji_id=(icon_emoji_id if client.aetheris_me.premium else None),
            )
        )
    ...
    forums_cache = db.pointer("aetheris.forums", "forums_cache", {})
...
async def wait_for_content_channel(db: "Database", delay: float = 10) -> int:
    cid = db.get("aetheris.forums", "channel_id", None)

    while not cid:
        logger.warning(
            "Aetheris content channel not found in database. Sleeping 10 seconds..."
        )
...
async def get_topic_id(db: "Database", topic_name: str) -> int | None:
    ...
    """
    try:
        forums_cache = db.get("aetheris.forums", "forums_cache", {})
        return forums_cache.get("aetheris-userbot", {}).get(topic_name)
    except Exception:
        return None
...
def get_chat_id(message: Message) -> int:
    ...
    :return: Chat ID
    """
    return aetheris_tl.utils.resolve_id(
...
def get_entity_id(entity: hints.Entity) -> int:
    ...
    :return: Entity ID
    """
    return aetheris_tl.utils.get_peer_id(entity)

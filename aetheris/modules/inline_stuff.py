...
class InlineStuff(loader.Module):
    ...
    @loader.command()
    async def ch_aetheris_bot(self, message: Message):
        ...
        self._db.set("aetheris.inline", "custom_bot", args)
    ...
    async def ch_bot_token(self, message: Message):
        ...
        if not args or not re.match(r"[0-9]{8,10}:[a-zA-Z0-9_-]{34,36}", args):
            ...
            return
        self._db.set("aetheris.inline", "bot_token", args)
        self._db.set("aetheris.inline", "needs_inline_setup", True)
        await utils.answer(message, self.strings["bot_updated"])
                ...
                await message.answer_photo(
                    ...
                    caption=self.strings["this_is_aetheris"].format(
                        (
                            ...
                            if self._client.aetheris_me.premium is True
                            else "🪐"
                        ),
                        utils.get_platform_emoji() if self._client.aetheris_me.premium is True else "Aetheris",
                    ),
                    reply_markup=self.inline.generate_markup(
                        markup_obj=[
                            [
                                {
                                    "text": "GitHub",
                                    "url": "https://github.com/coddrago/Aetheris",
    ...
    async def reset_prefix(self, call: InlineCall, message):
        ...
        self.db.set("aetheris.main", "command_prefix", ".")
